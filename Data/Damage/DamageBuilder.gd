...
# Starter identity tuning.
# Applies only when the player has exactly one unique element.
# Once the player starts combining elements, combo damage behaves normally.
const STARTER_FIRE_DAMAGE_MULT: float = 1.25
const STARTER_ICE_DAMAGE_MULT: float = 0.90
...
static func _get_starter_element_damage_multiplier(element_type: int, equipped_powerups: Array) -> float:
	if _get_unique_element_count(equipped_powerups) != 1:
		return 1.0

	match element_type:
		PowerUpData.ElementType.FIRE:
			return STARTER_FIRE_DAMAGE_MULT

		PowerUpData.ElementType.ICE:
			return STARTER_ICE_DAMAGE_MULT

		PowerUpData.ElementType.LIGHTNING:
			return STARTER_LIGHTNING_DAMAGE_MULT

		PowerUpData.ElementType.POISON:
			return STARTER_POISON_DAMAGE_MULT

		_:
			return 1.0


static func _get_unique_element_count(equipped_powerups: Array) -> int:
	var unique_elements: Array[int] = []
