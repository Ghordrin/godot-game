...
## Central damage builder.
## Keeps projectile scripts from owning all damage math.
...
static func build_projectile_packet(
	...
	for entry in equipped_powerups:
		...
		packet.add_debug(
			"[DMG] raw %s=%.1f base=%.1f amount=%.2f rank=%d wave_scale=%.2f" %
			[
	...
	var elemental_cap: float = base_damage * _get_element_cap_multiplier(safe_wave)
	...
	packet.add_debug(
		"[DMG] raw_element_total=%.1f cap=%.1f cap_scale=%.2f" %
		[raw_total, elemental_cap, cap_scale]
	)

	for element_type in raw_elements:
		var capped_amount: float = float(raw_elements[element_type]) * cap_scale
		var damage_type: String = _element_to_damage_type(int(element_type))
		...
		packet.add_debug(
			"[DMG] final %s=%.1f" %
			[
				PowerUpData.ElementType.keys()[element_type],
				capped_amount
			]
		)
...
static func _get_element_cap_multiplier(wave: int) -> float:
	if wave <= 5:
...
static func _element_to_damage_type(element_type: int) -> String:
	match element_type:
